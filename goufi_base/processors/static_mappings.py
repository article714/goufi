...
class Processor(CSVImporterMixin, XLImporterBaseProcessor):
    ...
    def process_tab_header(self, tab=None, headerrow=None):
        ...
        self.logger.info(
            "NEW SHEET:  Import data for model " + toString(self.target_model)
        )
        ...
        for val in header:
            ...
            if re.match(r"ID(.*)", hname):
                ...
                if hname in target_fields:
                    ...
                    self.logger.debug(
                        " %s -> field not found, IGNORED", toString(hname)
                    )
        ...
        if len(self.idFields) > 0:
            ...
            if len(found) == 1:
                ...
                self.logger.warning(
                    DEFAULT_LOG_STRING,
                    line_index,
                    "FOUND TOO MANY RESULT FOR %s with %s =>   [ %d ]"
                    % (
        ...
        if len(self.o2mFields) > 0 or len(self.m2oFields) > 0:
                    ...
                    if field[0] == 0 and data_values[f] and len(data_values[f]) > 0:
                        ...
                        if len(vals) == 1:
                            ...
                            self.logger.warning(
                                DEFAULT_LOG_STRING,
                                line_index,
                                " found  %d  values for %s unable to reference %s - %s "
                                % (
                                    len(vals),
            ...
            try:
                ...
                self.logger.error(
                    DEFAULT_LOG_STRING,
                    line_index,
                    " wrong values where creating/updating object: %s  -> %s [ %s ]"
            ...
            try:
                        ...
                        if len(members) > 0 and currentObj != None:
                            ...
                            for m in members:
                                if len(m) > 0:
                                    ...
                                    if field[0] == 0:
                                        vals = self.odooenv[field[2]].search(
                                            [(field[3], "=", m)], limit=1
                                        )
                                        if len(vals) == 1:
                                            currentObj.write(
                                                {field[1]: [(4, vals[0].id, False)]}
                                            )
                                        else:
                                            self.logger.warning(
                                                DEFAULT_LOG_STRING,
                                                line_index,
                                                "found %d  values for %s,  unable to reference"
                                                % (len(vals), toString(m)),
                                            )
                ...
                self.logger.error(
                    DEFAULT_LOG_STRING,
                    line_index,
                    " Wrong values where updating object: %s -> %s "
        ...
        # pas de champs collection
            ...
            try:
                ...
                self.logger.error(
                    DEFAULT_LOG_STRING,
                    line_index,
                    " error where creating/updating object: %s --> %s [%s]"
                    % (
                ...
                self.logger.error(
                    DEFAULT_LOG_STRING,
                    line_index,
                    " Generic Error : %s --- %s" % (toString(type(e)), toString(e)),
                )
                currentObj = None
    ...
    def get_row_values_as_dict(self, tab=None, row=None, tabheader=None):
        ...
        if self.fileType == FILE_TYPE_CSV:
            ...
            values = XLImporterBaseProcessor.get_row_values_as_dict(
                self, tab, row, tabheader
            )
    ...
    def process_file(self, import_file, force=False):
        ...
        if ext in AUTHORIZED_EXTS:
            ...
            self.end_processing(
                ...
                success=False,
                status="failure",
                any_message="Wrong file exension",
            )
