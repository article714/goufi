...
class ImportConfiguration(models.Model):
    ...
    files_location = fields.Char(
        string=_(u"Files Location"),
        help=_(
            u""" The place where we should try to find files that will be processed according to the current config
                                 """
        ),
        required=True,
        default="/odoo/file_imports",
    )

    recursive_search = fields.Boolean(
        string=_(u"Recursive search"),
        help=_(u"should we search for files in sub-directories"),
        default=False,
    )

    working_dir = fields.Char(
        string=_(u"Working directory"),
        help=_(u" Directory where temp and log files will be put"),
        required=True,
        default="/odoo/file_imports/_work",
    )

    default_header_line_index = fields.Integer(
        string=u"Default Header line",
    ...
    needs_partner = fields.Boolean(
        string=_(u"Needs partner"),
    ...
    needs_mappings = fields.Boolean(
        ...
        help=_(
            u"Does the selected processor need \
                                        column/tab mappings"
        ),
    ...
    has_parameters = fields.Boolean(
        ...
        help=_(
            u"Does the selected processor accept \
                                        parameters"
        ),
    ...
    def _get_default_language(self):
        ...
        strid = (
            ...
            .sudo()
            .get_param("goufi.goufi_default_language")
        )
        id = 0
        try:
            id = int(strid)
    ...
    processor_parameters = fields.One2many(
        string=_(u"Processor parameters"),
    ...
    column_mappings = fields.One2many(
        string=_(u"Column mappings"),
    ...
    single_mapping = fields.Boolean(
        string=_(u"Single mapping for all tabs"),
        help=_(
            u"Does the selected processor use \
                                        the same mappings for all tabs"
        ),
        related="processor.needs_mappings",
    )

    tab_mappings = fields.One2many(
        string=_(u"Tab mappings"),
        help=_(u"Mapping configuration needed by this processor"),
    ...
    def detect_files(self, cr=None, uid=None, context=None, cur_dir=None):
        ...
        delete_files_val = self.env["ir.config_parameter"].get_param(
            "goufi.delete_obsolete_files"
        )
        ...
        for aFile in all_existing_files:
            try:
                ...
                logging.exception(
                    u"Goufi failed to archive/delete import_file:%s" % aFile.filename
                )
        ...
        for aFile in all_files:
            ...
            if os.path.isdir(cur_path):
                ...
                if re.match(self.filename_pattern, aFile):
                    ...
                    existing_found = file_model.search(
                        [("filename", "=", cur_path), ("import_config", "=", self.id)]
                    )
                    ...
                    if nb_found == 1:
                        ...
                        logging.error(
                            "GOUFI: multiple import files found for :" + cur_path
                        )
