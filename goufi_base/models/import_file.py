...
class ImportFile(models.Model):
    ...
    # File identification
    filename = fields.Char(string=_(u'File name'),
    ...
    needs_partner = fields.Boolean(string=_(u"Needs partner"),
                                   help=_(
                                    "Does the selected configuration needs a partner reference"),
                                   related="import_config.needs_partner",
                                   read_only=True, store=False)

    date_addition = fields.Datetime(
        string=_(u"Addition date"), track_visibility='onchange', required=True)
    ...
    import_config = fields.Many2one(string=_(u'Related import configuration'),
                                    comodel_name='goufi.import_configuration',
                                    track_visibility='onchange', required=True)

    configuration_is_active = fields.Boolean(related='import_config.active')

    to_process = fields.Boolean(string=_(
        u"File has to be processed"), default=True, track_visibility='onchange')
    ...
    header_line_index = fields.Integer(string=_(u"Header line"), help=_(
        u"Fixes the index of the header line in import file"), default=1)
    ...
    processing_status = fields.Selection([('new', 'new'),
                                          ...
                                          (u'pending', u'pending'),
                                          (u'failure', u'failure')],
                                         string=_(u"Processing status"), default='new', 
                                         track_visibility='onchange')

    processing_result = fields.Text()

    log_filename = fields.Char(string=_(u'Log file name'),
                               help=('logfile name, used for download'),
                               store=False,
                               compute='_get_logfile_name')

    processing_logs = fields.Binary(string=u'Processing logs', 
                                    prefetch=False, attachment=False)
    ...
    # -------------------------------
    # file processing
    @api.depends('active', 'import_config', 'configuration_is_active', 'to_process', 
    'processing_status', 'date_start_processing', 'date_stop_processing', 'process_when_updated', 
    'date_updated', 'date_addition')
    def _file_needs_processing(self):
        for record in self:
            ...
            if record.active and record.configuration_is_active:
                ...
                result = result or (record.processing_status == 'pending') or (
                    record.processing_status == 'new')

                # file is to be processed not already being processed
                result = result and (record.to_process) and (
                    record.processing_status != 'running')

                # File is active and config also
                if len(record.import_config) > 0:
                    result = result and (record.active) and (
                        record.import_config.active)
                else:
                    result = False
    ...
    def process_file(self):
        for aFile in self:
            if aFile.needs_to_be_processed:
                ...
                if lang:
                    aFile.with_context({'lang': lang.code}
                                       )._process_a_file(force=False)
                else:
    ...
    def _process_a_file(self, force=False):
        ...
        if self.import_config:
            ...
            try:
                ...
                if cls is None:
                    logging.error(
                        "GOUFI: Cannot process file, no processor class found " + self.filename)
                    return None
            ...
            if mod and cls:
                try:
                    ...
                    logging.exception(
                        "Not Able to Process import File %s", self.filename)
    ...
    def process_files(self, criteria=[], maxFiles=10, anOrder=None):
        ...
        if import_file_model is not None:
            records = ()
            if anOrder is None:
                records = import_file_model.search(criteria, limit=maxFiles)
            else:
                records = import_file_model.search(
                    criteria, order=anOrder, limit=maxFiles)
            for rec in records:
                rec.process_file()
    ...
    def _compute_display_name(self):
        for record in self:
            if record.filename and record.import_config:
                record.display_name = '[' + record.import_config.name + \
    ...
    def create(self, values):
        if 'import_config' in values:
            config = self.env['goufi.import_configuration'].browse(
                (values['import_config'],))
            if len(config) > 0:
                config = config[0]
