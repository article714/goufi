...
class ImportFile(models.Model):
    ...
    filename = fields.Char(
        string=_(u"File name"), required=True, track_visibility="onchange"
    )
    ...
    import_config = fields.Many2one(
        string=_(u"Related import configuration"),
    ...
    needs_to_be_processed = fields.Boolean(
        string=_(u"File needs to be processed"),
        compute="_file_needs_processing",
        store=True,
    )

    process_when_updated = fields.Boolean(
        string=_(u"File is to be re-processed when updated"),
        default=True,
        track_visibility="onchange",
    )
    ...
    log_filename = fields.Char(
        string=_(u"Log file name"),
        help=("logfile name, used for download"),
        store=False,
        compute="_get_logfile_name",
    )
    processing_logs = fields.Binary(
        string=u"Processing logs", prefetch=False, attachment=False
    )
    ...
    @api.depends(
        ...
        "date_stop_processing",
        "process_when_updated",
        "date_updated",
        "date_addition",
    )
        ...
        for record in self:
            ...
            if record.active and record.configuration_is_active:
                ...
                if record.process_when_updated:
                    ...
                    result = (
                        (lastproc_time > 0)
                        and (upd_time > lastproc_time)
                        and (record.processing_status != "running")
                    )

                # File is New or process is waiting for processing
                result = (
                    result
                    or (record.processing_status == "pending")
                    or (record.processing_status == "new")
                )

                # file is to be processed not already being processed
                result = (
                    result
                    and (record.to_process)
                    and (record.processing_status != "running")
                )
    ...
    def _process_a_file(self, force=False):
        ...
        if self.import_config:
            ...
            try:
                ...
                if cls == None:
                    logging.error(
                        "GOUFI: Cannot process file, no processor class found "
                        + self.filename
                    )
                    return None
            except Exception as e:
                logging.error(
                    "GOUFI: Cannot process file, error when evaluating processor module"
                    + self.filename
                    + "("
                    + str(e)
                    + ")"
