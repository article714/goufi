...
class ColumnMapping(models.Model):
    ...
    _sql_constraints = [
        (
            "unique_column_name",
            "UNIQUE(name,parent_configuration,parent_tab)",
            _(u"Name of the column must be unique per configuration/tab \n"),
        )
    ]
    ...
    sequence = fields.Integer(
        string=_(u"Sequence"),
        default=1,
        help=_(u"Used to order mappings. Lower is better."),
    )

    # expression
    mapping_expression = fields.Char(
        string=_(u"Mapping Expression"),
        help=_(
            u"Expression used to process column content, meaning depends on chosen processor."
        ),
        required=False,
        track_visibility="onchange",
    )
    ...
    # expression
    preprocess_expression = fields.Char(
        string=_(u"Data Pre-processing"),
        help=_(
    ...
    is_identifier = fields.Boolean(
        string=_(u"Is column part of identifiers?"),
        help=_(
            u"""The value of the mapping is used to find existing records in Odoo database.
...
"""
    ...
    is_change_marker = fields.Boolean(
        string=_(u"Does column contain a change marker?"),
        help=_(u"If True, the selected record (if found) will be deleted"),
        default=False,
    )
    update_if_expression = fields.Char(
        string=_(u"Update if value matches"),
        help=_(
    ...
    is_deletion_marker = fields.Boolean(
        string=_(u"Does column contain a deletion marker?"),
        help=_(u"If True, the selected record (if found) will be deleted"),
        default=False,
    )

    delete_if_expression = fields.Char(
        string=_(u"Delete if value matches"),
        help=_(
    ...
    archive_if_not_deleted = fields.Boolean(
        string=_(u"Archive if not deleted?"),
        help=_(u"Should we archive record if deletion fail?"),
        default=False,
    )

    # is column an archival marker
    is_archival_marker = fields.Boolean(
        string=_(u"Does column contain an archival marker?"),
        help=_(u"If True, the selected record (if found) will be archived"),
        default=False,
    )

    archive_if_expression = fields.Char(
        string=_(u"Archive if value matches"),
        help=_(
    ...
    is_constant_expression = fields.Boolean(
        string=_(u"Expression is a constant"),
    ...
    is_function_call = fields.Boolean(
        string=_(u"Expression is a function to call"),
        help=_(
            u"""The value assigned to target property is the result of a function call.
...
Function must return the value to be assigned to mapping or None"""
    ...
    is_contextual_expression_mapping = fields.Boolean(
        string=_(u"Is a contextual expression mapping "),
        help=_(
    ...
    target_object = fields.Many2one(
        string=_(u"Target object"),
        help=_(
            u"Odoo object that will be targeted by import: create, update or delete instances"
        ),
    ...
    target_field = fields.Many2one(
        string=_(u"Target field"),
        help=_(
            u"Odoo object's field that will be targeted by import: create, update or delete instances"
        ),
    ...
    parent_configuration = fields.Many2one(
        string=_(u"Parent configuration"), comodel_name="goufi.import_configuration"
    )

    tab_support = fields.Boolean(
        string=_(u"Supports multi tabs"),
        help=_(
            u"Does the selected parent configuration's pocessor can process multiple tabs"
        ),
        related="parent_configuration.tab_support",
        store=False,
    )

    parent_tab = fields.Many2one(
        string=_(u"Parent Tab (when multi tabs)"), comodel_name="goufi.tab_mapping"
    )

    # computed field

    display_target = fields.Char(
        string=_(u"Target Field"),
        help=_(u"Target field for target object"),
        required=False,
        store=False,
    ...
    def _get_target_object(self):
        for colMap in self:
            ...
            if colMap.parent_tab:
                if (
                    colMap.parent_configuration
                    and colMap.parent_configuration
                    != colMap.parent_tab.parent_configuration
                ):
        ...
        for colMap in self:
            if len(colMap.target_object) > 0:
                if len(colMap.target_field) > 0:
                    colMap.display_target = (
                        colMap.target_object.model + "." + colMap.target_field.name
                    )
                else:
        ...
        try:
            ...
            if "parent_tab" in values:
                if values["parent_tab"] != None:
                    found = self.env["goufi.tab_mapping"].search(
                        [("id", "=", values["parent_tab"])], limit=1
                    )
                    if len(found) == 1:
                        values["parent_configuration"] = found[
                            0
                        ].parent_configuration.id
        except Exception as e:
            logging.exception(
                "Not able to check values when creating column mapping %s : %s"
